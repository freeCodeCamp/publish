...
const PostForm = ({ tags, user, authors, post }) => {
  ...
  function handlePublishDateChange(event) {
    const date = new Date(event.target.value);
    let formattedDate = date.toISOString();

    // if time is not set it will default to the date at midnight

    const hasTime = publishTime !== '';

    if (hasTime) {
      let newTime = formattedDate.split('T')[0] + 'T' + publishTime;
  ...
  function handlePublishTimeChange(event) {
    const [hours, minutes] = event.target.value.split(':');
    const time = new Date();
    time.setHours(hours);
    time.setMinutes(minutes);

    // defaults to the current secodns and milliseconds of the current time
    // Setting to 0 to avoid issues with the publish date
    ...
    const newDateTime = publishDate.split('T')[0] + 'T' + formattedTime;
  ...
  function handleDateTimeOnInit() {
    ...
    // formatted time without seconds and milliseconds
    const formattedTime =
      date.toISOString().split('T')[1].split(':')[0] +
      ':' +
      date.toISOString().split('T')[1].split(':')[1];

    // Setting the local time in the time input field
    const localDate = new Date(publishDate ? publishDate : Date.now());

    // Adds timezone offset going from UTC to local time e.g. JST is UTC+9 so it will add 9 hours
    localDate.setMinutes(
      localDate.getMinutes() - localDate.getTimezoneOffset()
    );

    const formattedLocalTime =
      localDate.toISOString().split('T')[1].split(':')[0] +
      ':' +
      localDate.toISOString().split('T')[1].split(':')[1];

    document.getElementById('publish-date').value = formattedDate;
    document.getElementById('publish-time').value = formattedLocalTime;
